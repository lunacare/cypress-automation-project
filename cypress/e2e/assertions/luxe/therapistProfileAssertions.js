...
cy.contains(therapistProfilePage.phone_number).parent().next(console.log)

cy.contains(therapistProfilePage.legal_address).parent().next().invoke('text').should('contains',"55 Golden St, Bay Area, CA, 92345")
//cy.contains(therapistProfilePage.status).parent().next().invoke('text').should('equal','Draft')
